//! Query runtime layout without creating directories.
...
pub struct QueryRuntimeLayout<'a, R> {
    pub resolver: &'a R,
}
...
mod tests {
    ...
    #[test]
    fn resolves_read_only_layout() {
        let layout = fixture_layout("/tmp/tentgent-layout-query");
        let resolver = FakeLayoutResolver {
            layout: layout.clone(),
        };

        let output = QueryRuntimeLayout::new(&resolver)
            .run()
            .expect("query layout");
    ...
    pub(super) fn fixture_layout(root: &str) -> RuntimeLayout {
