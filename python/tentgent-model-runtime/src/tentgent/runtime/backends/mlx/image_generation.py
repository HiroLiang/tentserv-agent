...
class MfluxImageGenerationModel(MlxBackendModel, ImageGenerationBackendModel):
    def __init__(self) -> None:
        self._deps = _load_mflux_deps()
        self._record: ModelRecord | None = None
        self._model: Any | None = None
        self._model_workflow: ImageGenerationWorkflowKind | None = None
    ...
    def generate_image(self, request: ImageGenerationRequest) -> ImageGenerationResult:
        if request.workflow_kind == ImageGenerationWorkflowKind.CONTROL:
            raise NotImplementedError(
                "MFLUX image control / ControlNet generation is not supported."
            )
        model = self._load_model(request.workflow_kind)
        seed = request.seed
        if seed is None:
            seed = random.SystemRandom().randrange(0, 2**63)
        output_request = replace(request, seed=seed)
        if request.workflow_kind == ImageGenerationWorkflowKind.INPAINT:
            if request.negative_prompt:
                raise RuntimeError("MFLUX inpainting does not support negative prompts.")
    ...
    def _load_model(self, workflow: ImageGenerationWorkflowKind) -> Any:
        if self._record is None:
            raise RuntimeError(
                "MFLUX image generation model is not loaded yet; call load() first."
            )
        ...
        model_class = (
            self._deps.Flux1Fill
            if workflow == ImageGenerationWorkflowKind.INPAINT
            else self._deps.Flux1
        )
        self._model = model_class(
            model_config=model_config,
            quantize=quantize,
            model_path=str(self._record.source_path),
...
def _load_mflux_deps() -> _MfluxDeps:
    try:
        ...
        raise RuntimeError(
            "MFLUX image generation requires a recent runtime with Flux "
            "text-to-image and Flux Fill API support."
        ) from exc

    return _MfluxDeps(Flux1=Flux1, Flux1Fill=Flux1Fill, ModelConfig=ModelConfig)


def _mflux_flux_model_config(record: ModelRecord, model_config: Any) -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            "MFLUX image generation supports Flux-family MLX models, but model "
            f"`{source_repo}` could not be mapped to a Flux base model: {exc}"
        ) from exc
...
def _mflux_quantize_bits(record: ModelRecord) -> int | None:
    label = " ".join(
        value
        for value in [record.source_repo, str(record.source_path)]
        if value
    ).lower()
...
def _ensure_mflux_fill_model(record: ModelRecord) -> None:
    label = " ".join(
        value
        for value in [record.source_repo, str(record.source_path)]
        if value
