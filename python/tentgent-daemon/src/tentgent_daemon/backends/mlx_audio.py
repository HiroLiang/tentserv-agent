...
class MlxAudioTranscriptionBackend(AudioTranscriptionBackend):
    ...
    def load(self, record: StoredModelRecord) -> None:
        self._record = record
        self._model = self._deps.load(str(record.variant_source_path))
    ...
    def _require_loaded(self) -> Any:
        if self._record is None or self._model is None:
            raise RuntimeError(
                "MLX audio transcription backend is not loaded yet; "
...
def _load_mlx_audio_deps() -> MlxAudioDeps:
    try:
        ...
        if exc.name and exc.name.startswith("mlx_audio"):
            raise missing_profile_dependency("local-model", "mlx-audio") from exc
        if exc.name == "mlx":
            raise missing_profile_dependency("local-model", exc.name) from exc
        raise

    return MlxAudioDeps(load=load)
...
def _generate_kwargs(
    ...
    if request.timestamps or request.output_format in {"vtt", "srt"}:
        candidates["word_timestamps"] = True

    return _supported_kwargs(generate, candidates, language_requested=bool(request.language))
...
def _is_known_language_error(error: Exception) -> bool:
...
def _is_missing_processor_metadata_error(error: Exception) -> bool:
    message = str(error).lower()
    return "processor not found" in message or (
        "can't load feature extractor" in message
        and "preprocessor_config.json" in message
    )


def _is_unexpected_keyword_error(error: Exception) -> bool:
    message = str(error).lower()
    return isinstance(error, TypeError) and "unexpected keyword" in message
