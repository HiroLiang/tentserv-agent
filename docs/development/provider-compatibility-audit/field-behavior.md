...
| `contents` | Required by Gemini-shaped chat routes. | Daemon and local model-bound routes accept text parts only. Direct cloud route accepts text plus `inlineData` and `inline_data` image/audio parts. |
