...
| --- | --- | --- | --- | --- |
| Chat | Partial. `/v1/chat/completions` supports text chat fields on the daemon and local model-bound servers; direct cloud servers can also translate OpenAI image parts for compatible models. | Partial. `/v1/messages` supports text Claude-style messages on daemon and local model-bound servers; direct cloud servers can translate text and base64 image blocks for compatible models. | Partial. `generateContent` supports text Gemini-style contents on daemon and local model-bound servers; direct cloud servers can translate text and inline image data for compatible models. | `/v1/chat` for native local text chat. |
