...
| Local model-bound server | `tentgent server run <model-ref>` | `/v1/chat`, `/v1/chat/completions`, `/v1/embeddings`, and native capability routes | Native routes stay Tentgent-shaped. Implemented provider-shaped ingress routes translate requests into native local runtime calls and wrap responses back into the provider shape. |
