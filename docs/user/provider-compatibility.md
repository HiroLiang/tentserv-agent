...
| Cluster server | `tentgent cluster run <cluster-ref>` | Provider-shaped text chat routes plus native/local `/v1/chat`, `/v1/chat/stream`, `/v1/embeddings`, `/v1/rerank`, `/v1/audio/transcriptions`, and `/v1/vision/chat` | The endpoint family selects the declared cluster route. Request `model` values do not override the cluster target. Only local-model targets execute in the first MVP; provider targets return `cluster_route_target_unsupported`. |

## Endpoint Families

| Endpoint family | OpenAI-compatible | Claude/Anthropic-compatible | Gemini-compatible | Native Tentgent fallback |
| --- | --- | --- | --- | --- |
| Chat | Partial. `/v1/chat/completions` supports text chat fields on the daemon and local model-bound servers; direct OpenAI cloud servers can also translate OpenAI image parts and non-streaming audio chat for compatible models. | Partial. `/v1/messages` supports text Claude-style messages on daemon and local model-bound servers; direct cloud servers can translate text and base64 image blocks for compatible models. Claude-compatible audio blocks and audio-shaped fields are unsupported on all Tentgent Claude routes today. | Partial. `generateContent` supports text Gemini-style contents on daemon and local model-bound servers; direct cloud servers can translate text, inline image data, and inline audio data for compatible models. | `/v1/chat` for native local text chat. |
...
| Image generation | Partial. `/v1/images/generations` accepts `model`, `prompt`, and optional `size` on daemon, direct cloud, and image-generation local model-bound servers. | Unsupported. Anthropic image generation is not implemented. | Partial. `/v1/images/generations` can route Gemini image model requests through Gemini `generateContent` and Imagen requests through Gemini `predict` where the provider model supports image generation. | `/v1/images/generations/job` for daemon local image-generation workflows; local model-bound image-generation servers also accept native `/v1/images/generations` bodies with `output_path`. |
| Audio transcription | Partial. Direct OpenAI cloud chat accepts `input_audio`; OpenAI-shaped `/v1/audio/transcriptions` is not implemented, and daemon/local chat still reject `input_audio`. | Unsupported. Claude-compatible message audio blocks and `input_audio` fields fail before runtime or upstream dispatch. | Partial. Direct cloud Gemini `generateContent` can pass inline audio data to compatible Gemini models; daemon/local Gemini routes and dedicated audio routes remain unsupported. | `/v1/audio/transcriptions/job`. |
| Audio speech | Partial. Direct OpenAI cloud chat accepts non-streaming `audio` output options; OpenAI-shaped `/v1/audio/speech` is not implemented, and daemon/local chat still reject audio output options. | Unsupported. Claude-compatible `audio` and `modalities` fields fail before runtime or upstream dispatch. | Unsupported. | `/v1/audio/speech/job`. |
| Vision chat | Partial. Direct cloud chat servers can pass image parts to compatible OpenAI models; daemon and local model-bound OpenAI chat compatibility remain text-only. | Partial. Direct cloud chat servers can pass base64 image blocks to compatible Claude models; daemon chat compatibility remains text-only. | Partial. Direct cloud chat servers can pass inline image data to compatible Gemini models; daemon and local model-bound Gemini compatibility remain text-only. | `/v1/vision/chat` for one local image plus one prompt. |
...
| Image size | Supported as `size` on provider-compatible image generation. | Unsupported. | Partial. Gemini image models accept `size` for route parity but do not forward it today; Imagen fallback forwards size as `sampleImageSize`. | Native local image jobs use `width` and `height`; local OpenAI image ingress maps `size` to `width` and `height`. |
