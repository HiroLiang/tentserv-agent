...
| `experimental` | `GET /v1/doctor` | Public diagnostic route. Output remains compact and actionable, but warning coverage expands during `v0.9.0`. |
...
| `experimental` | `POST /v1/daemon/shutdown` | Public local shutdown request. Cleanup guarantees are finalized by the `v0.9.0` shutdown hardening slice. |
...
| `experimental` | `POST /v1/video/understanding/job`, `GET /v1/video/understanding/job/{job_id}/result` | Durable video workflow. Route shape is public; decoder/runtime support and stale cleanup remain hardening targets. |
| `experimental` | `POST /v1/images/generations/job`, `GET /v1/images/generations/job/{job_id}/files`, `GET /v1/images/generations/job/{job_id}/files/{file_id}` | Durable local text-to-image workflow. Artifact route shape is public; runtime support and cleanup semantics remain hardening targets. |
...
| `experimental` | `GET /v1/jobs`, `GET /v1/jobs/{job_id}`, `DELETE /v1/jobs/{job_id}`, `POST /v1/jobs/{job_id}/cancel` | Public job control surface. Cancellation and cleanup guarantees are finalized by the `v0.9.0` cleanup hardening slice. |
...
| `experimental` | `POST /v1/models/{reference}/capabilities`, `GET /v1/models/{reference}/capabilities/proofs`, `POST /v1/models/{reference}/capabilities/verify` | Public support-status and proof operations. Retry and stale-state behavior is finalized by the support-proof hardening slice. |
...
| `experimental` | `POST /v1/train/lora/plans/{reference}/runs`, `GET /v1/train/lora/plans/{reference}/runs`, `GET /v1/train/lora/runs`, `GET /v1/train/lora/runs/{reference}`, `GET /v1/train/lora/runs/{reference}/metrics`, `GET /v1/train/lora/runs/{reference}/logs`, `GET /v1/train/lora/runs/{reference}/logs/raw` | Managed LoRA run surface. Run cleanup, stale process handling, and diagnostics remain hardening targets. |
...
| Tier | Commands | Notes |
| --- | --- | --- |
| `stable` | `tentgent doctor`, `tentgent runtime bootstrap`, `tentgent runtime status` | Runtime and diagnostic commands. Doctor coverage expands during `v0.9.0`. |
...
| `experimental` | `tentgent model capability proofs`, `model capability verify`, `model capability proof clear` | Public support-proof commands. Retry and stale-state behavior is finalized by the support-proof hardening slice. |
...
| `experimental` | `tentgent train lora plan create/ls/inspect/rm`, `tentgent train lora run` | Public LoRA training commands. Plan identity is contracted; run behavior and stale recovery remain hardening targets. |
...
The active `v0.9.0` audit findings are tracked in
[v0.9.0-api-surface-audit-findings.md](../plans/v0.9.0-api-surface-audit-findings.md).
That file records wording gaps, test follow-ups, and any required behavior
changes discovered while maintaining this contract.
