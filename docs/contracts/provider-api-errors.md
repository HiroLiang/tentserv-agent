...
| `unsupported_provider_content` | A provider-shaped message, content part, or block uses a content type that this route cannot translate yet. | OpenAI daemon/local `image_url` parts, malformed direct cloud OpenAI `image_url` parts, Claude daemon/local `image`, `tool_use`, and `tool_result` blocks, Gemini daemon/local non-text parts, unknown direct cloud multimodal part shapes. |
