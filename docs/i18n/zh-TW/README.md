...
Tentgent 目前仍是 source-first。它還不適合正式 release 給一般使用者，因為 packaged Python runtime bootstrap 還在整理中。

目前可用：
- 從此 repository build 並執行
- 使用 `scripts/package-local.sh` 建立本機 release-like smoke-test tarball
- 當本機已經有 `uv` 時，用 `tentgent doctor --fix` 作為 developer bootstrap 路徑

之後規劃：
- installer-managed Python bootstrap，不能要求使用者預先安裝 `uv`
- Homebrew 安裝
...
建立本機 release-like smoke-test artifact：

```bash
scripts/package-local.sh
```

此 script 會寫入 `dist/tentgent-<version>-<target>.tar.gz` 與 `dist/checksums.txt`。
這個 artifact 適合測試安裝 layout，但在 installer 能自行處理 Python runtime bootstrap、且不依賴使用者預裝 `uv` 前，不應作為一般使用者 release。

若只想 smoke-test installer layout，不下載大型 Python ML dependencies：
...
拿掉 `--skip-python-bootstrap` 會執行完整 managed Python bootstrap：下載 pinned `uv` 到 Tentgent bootstrap cache、建立 `TENTGENT_HOME/runtime/python-env`，並安裝 Python ML dependencies。
...
開發時若本機已有 `uv`，可建立或同步 managed Python environment：
...
- `tentgent doctor` 會執行安裝與 runtime 健康檢查。
- `tentgent doctor --fix` 是 developer-only bootstrap 路徑，目前仍需要 `uv`。
- `tentgent status` 會顯示目前平台與 backend capability 狀態。
- `safetensors` 模型會在 Python 依賴安裝完成後透過 `transformers-peft` backend 執行。
- `mlx` 模型只在 Apple Silicon macOS 上透過 MLX backend 執行。
- `gguf` 模型會在 `llama-cpp-python` 依賴安裝完成後執行。
...
- Windows 已列入規劃，但還不是完整支援的 release target。MLX 會在 Windows 上被阻擋。
